...
namespace Foundation.DependencyInjection.Extensions
{
    ...
    public static class ServiceCollectionExtensions
    {
        ...
        {
            var typesWithAttributes = assemblies.Where(assembly => !assembly.IsDynamic).SelectMany(GetExportedTypes)
                .Where(type => !type.IsAbstract && !type.IsGenericTypeDefinition).Select(
                    type => new
                    {
                        ServiceType = type,
            ...
            foreach (var type in typesWithAttributes)
            {
                if (type.ImplementationType == null)
                    serviceCollection.AddTransient(type.ServiceType);
                else
                    serviceCollection.AddTransient(type.ImplementationType, type.ServiceType);
            }
        }
